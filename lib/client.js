window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function formatTokensShort$1(n) {
			if (n >= 1e9) return (n / 1e9).toFixed(2) + "B";
			if (n >= 1e6) return (n / 1e6).toFixed(2) + "M";
			if (n >= 1e3) return (n / 1e3).toFixed(2) + "k";
			return n.toLocaleString();
		...
		function tooltipTotal(day) {
			...
			return `${day.dayKey}: ${formatTokensShort$1(day.totalTokens)} tokens`;
		}
		...
		function HourStrip({ day, hourLevels }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: hourly.map((v, h) => {
					...
					const hh = v === 0 ? `${String(h).padStart(2, "0")}:00 — No usage` : `${String(h).padStart(2, "0")}:00 — ${formatTokensShort$1(v)} tokens`;
					return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						title: hh,
						"aria-label": hh,
						style: {
					...
					children: cells.map((d, idx) => {
						...
						return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								children: d.totalTokens > 0 ? formatTokensShort$1(d.totalTokens) : ""
							})]
						}, d.dayKey);
					})
				})]
							...
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										...
										children: d.totalTokens === 0 ? "No usage" : `${formatTokensShort$1(d.totalTokens)} tokens`
									})
								]
							}, d.dayKey);
						})
		...
		*/
		...
		function recentDayKeys(endKey, n) {
			const out = [];
			for (let i = n - 1; i >= 0; i--) out.push(addDays(endKey, -i));
			return out;
		}
		/** Sum each model's tokens over `dayKeys`. */
		function modelTotals(days, dayKeys) {
			const inWindow = new Set(dayKeys);
			const totals = /* @__PURE__ */ new Map();
			for (const d of days) {
				if (!inWindow.has(d.dayKey)) continue;
				for (const [model, tokens] of d.byModel) totals.set(model, (totals.get(model) ?? 0) + tokens);
			}
			return totals;
		}
		const byTotalDesc = (a, b) => b[1] - a[1] || (a[0] < b[0] ? -1 : 1);
		/** Sum each model's tokens over `dayKeys`, return up to `k` names by total desc. */
		function pickTopModels(days, dayKeys, k = 5) {
			return [...modelTotals(days, dayKeys).entries()].sort(byTotalDesc).slice(0, k).map(([model]) => model);
		}
		/**
		...
		*/
		function pickTrendModels(days, dayKeys, weekKeys, k = 10, weekTopN = 3) {
			...
			const selected = new Set(weekTop);
			for (const m of rangeRanked) {
				if (selected.size >= k) break;
				selected.add(m);
			}
			return [...selected].sort((a, b) => (rangeTotals.get(b) ?? 0) - (rangeTotals.get(a) ?? 0) || (a < b ? -1 : 1));
		}
		/** One series per model: its daily tokens aligned to `dayKeys`, 0 where absent. */
		function buildSeries(days, dayKeys, models) {
		...
		*/
		...
		function seriesPath(points) {
			...
			return head + monotoneSegments(points).map((s) => ` C ${fmtNum(s.c1[0])} ${fmtNum(s.c1[1])}, ${fmtNum(s.c2[0])} ${fmtNum(s.c2[1])}, ${fmtNum(s.p1[0])} ${fmtNum(s.p1[1])}`).join("");
		}
		...
		*/
		function TrendChart({ t, days, isEn = false }) {
			...
			const topModels = react.useMemo(() => pickTrendModels(days, dayKeys, weekKeys, 10, 3), [
				days,
				dayKeys,
				weekKeys
			]);
			const series = react.useMemo(() => buildSeries(days, dayKeys, topModels), [
				days,
				dayKeys,
				topModels
			]);
			...
			const colorOf = (model) => COLORS[Math.max(0, topModels.indexOf(model)) % COLORS.length];
			...
			const xOf = (i) => PAD.left + (dayKeys.length > 1 ? i * step : plotW / 2);
			const yOf = (v) => PAD.top + (1 - v / yMax) * plotH;
			const fmtDay = (k) => `${Number(k.slice(5, 7))}/${Number(k.slice(8, 10))}`;
			const fmtDayLong = (k) => isEn ? `${Number(k.slice(5, 7))}/${Number(k.slice(8, 10))}` : `${Number(k.slice(5, 7))}月${Number(k.slice(8, 10))}日`;
			...
			const hoverRows = hover == null ? [] : visible.map((s) => ({
				model: s.model,
			...
			const gap = 14;
			const svgW = Math.max(0, wrap.w - 16);
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								30,
								90
							].map((n) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								onClick: () => {
									setRangeDays(n);
									setHover(null);
								},
								...
								children: n === 7 ? t("trend.range7") : n === 30 ? t("trend.range30") : t("trend.range90")
							}, n))
						})]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									...
									children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											...
											children: formatTokensShort(totalsByModel.get(s.model) ?? 0)
										})
									]
								}, s.model);
							})
						}), series.length === 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							style: {
								fontSize: 10,
								color: "var(--dsw-alias-label-tertiary)",
								marginTop: 4
							},
							children: t("trend.legend.hint")
						})]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						ref: wrapRef,
						style: {
							position: "relative",
							padding: `4px ${CX_PAD}px 12px`
						},
						children: [topModels.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							style: {
								padding: 24,
								textAlign: "center",
								fontSize: 14,
							...
							children: [
								[
									...
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("line", {
										...
										strokeWidth: 1,
										strokeDasharray: f === 0 ? void 0 : "3 3"
										...
										children: formatTokensShort(yMax * f)
									})] }, f);
								}),
								tickIdx.map((i) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("text", {
									x: xOf(i),
									y: 270,
									textAnchor: "middle",
									fontSize: 14,
									fill: "var(--dsw-alias-label-tertiary)",
									children: fmtDay(dayKeys[i])
								}, i)),
								...
								visible.map((s) => {
									...
									const pts = s.points.map((v, i) => [xOf(i), yOf(v)]);
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", {
										...
										}), pts.map(([x, y], i) => s.points[i] > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("circle", {
											cx: x,
											cy: y,
											r: hover?.idx === i ? 4 : 2,
											fill: color
								...
								visible.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("text", {
									x: W / 2,
									y: PAD.top + plotH / 2,
									textAnchor: "middle",
									fontSize: 13,
						...
						}), hover != null && topModels.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							style: {
								position: "absolute",
								left: tipLeft,
								top: tipTop,
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: fmtDayLong(dayKeys[hover.idx])
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									style: {
										display: "flex",
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: isEn ? "Total" : "消耗总量" }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [formatTokensShort(hoverTotal), " tokens"] })]
								}),
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: hoverRows.map((r) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										style: {
											display: "flex",
											alignItems: "center",
											gap: 6
										...
										children: [
											...
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
												...
												children: [formatTokensShort(r.value), " tokens"]
											})
										]
									}, r.model))
								})
		...
		function SettingsSection({ t, ctx, days: injectedDays, totals: injectedTotals, top5: injectedTop5 }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					allDays.some((d) => d.totalTokens > 0) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(TrendChart, {
						t: lang === "en" ? (k, p) => en[k] ?? t(k, p) : t,
						days: allDays,
						isEn: lang === "en"
					}) : null
				]
